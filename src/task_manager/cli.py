...
def create_task(title, description, due, priority, tag, project):
    ...
    try:
        t = manager.create_task(title=title, description=description, due=due_dt, priority=priority, tags=list(tag), project_name=project)
...
def list_tasks(project, tag):
    ...
    for t in sorted(tasks, key=lambda x: (x.status, x.priority)):
        ...
        click.echo(f"{t.id[:8]} | {t.title} | {t.status} | due:{due} | prio:{t.priority}")
...
def complete_task(task_id):
    try:
        t = manager.mark_complete(task_id)
